with jn as (
    select
        ...
        ltrim(d.code, '0') as db_databaseNumber,
        --processes
        pm.id as ggp_id,
        ...
        rh.name as ggp_remoteDatabaseNumber
    from si_tp_block b
    ...
    left join si_tp_ggdatabase rd on rd.id = pm.REMOTEDATABASEID
    left join si_tp_ggdbhost rh on rd.hostid = rh.id
    join si_tp_ggproject p on pm.projectid = p.id
    where p.name = 'UFS_REQUEST'),
prjct as (
...
db as (
    select distinct db_id, db_blockid, db_hostid, db_host, db_port, db_sid, db_description, db_databaseNumber, 
                    host_id
    from jn
),
ggp as (select distinct ggp_id, ggp_processname, ggp_processtype, ggp_description, ggp_databaseid, ggp_projectid, ggp_remoteDatabaseNumber, 
                        db_id
        from jn)
select
    xmlelement("project",
                                        ...
                                        xmlelement("databases",
                                            (select xmlagg(
                                                        xmlelement("databases",
                                                            ...
                                                            xmlelement("databaseNumber", db_databaseNumber)
                                                            ,
                                                            xmlelement("processes",
                                                                (select xmlagg(
                                                                            xmlelement("process",
                                                                                ...
                                                                                xmlelement("remoteDatabaseNumber", ggp_remoteDatabaseNumber)
                                                                            )
                                                                        )    
                                                                 from ggp
